...
static float trilinearInterpolation(
    matrix_t const& matrix, float const x, float const y, float const z)
{
  ...
  return (1 - xf) * (1 - yf) * (1 - zf) * v1 + xf * (1 - yf) * (1 - zf) * v2
         + (1 - xf) * yf * (1 - zf) * v3 + xf * yf * (1 - zf) * v4
         + (1 - xf) * (1 - yf) * zf * v5 + xf * (1 - yf) * zf * v6
         + (1 - xf) * yf * zf * v7 + xf * yf * zf * v8;
}

static std::vector<Position>
mbapPositionsFromSpeakers(MbapSpeaker const* speakers, size_t const num)
{
  std::vector<Position> positions{};
  positions.reserve(num);
  std::transform(
      speakers, speakers + num, std::back_inserter(positions),
      [](MbapSpeaker const& speaker) { return speaker.position; });
  return positions;
}
  ...
  for(size_t i{}; i < field.speakerPositions.size(); ++i)
  {
    ...
    for(size_t x{}; x < MBAP_MATRIX_SIZE; ++x)
    {
      for(size_t y{}; y < MBAP_MATRIX_SIZE; ++y)
      {
        ...
        {
          auto dist = std::sqrt(
              std::pow(narrow<float>(x) - px, 2.0f)
              + std::pow(narrow<float>(y) - py, 2.0f)
              + std::pow(narrow<float>(z) - pz, 2.0f));

          dist = std::pow(std::pow(10.0f, 1.0f / 20), dist);
          field.amplitudeMatrix[i][x][y][z] = 1.0f / std::sqrt(dist);
        }
        field.amplitudeMatrix[i][x][y][MBAP_MATRIX_SIZE]
            = field.amplitudeMatrix[i][x][y][0];
      }
...
static void computeGains(MbapField const& field, SourceData const& source, float* gains)
{
  ...
  auto const finalElevSpanExponent{
      ((sourceElevationSpan - EXPONENT_MIN_IN) * (EXPONENT_MAX_OUT - EXPONENT_MIN_OUT)
       / (EXPONENT_MAX_IN - EXPONENT_MIN_IN))
      + EXPONENT_MIN_OUT};

  auto const finalAzimuthSpanExponent{
      ((sourceAzimuthSpan - EXPONENT_MIN_IN) * (EXPONENT_MAX_OUT - EXPONENT_MIN_OUT)
       / (EXPONENT_MAX_IN - EXPONENT_MIN_IN))
      + EXPONENT_MIN_OUT};
  ...
  {
    auto squaredDistX
        = field.speakerPositions[i].getCartesian().x - source.position->getCartesian().x;
    squaredDistX *= squaredDistX;
    auto squaredDistY
        = field.speakerPositions[i].getCartesian().y - source.position->getCartesian().y;
    squaredDistY *= squaredDistY;
    distZ = std::abs(
        field.speakerPositions[i].getCartesian().z - source.position->getCartesian().z);
    ...
    auto const azimuthElevationGain{
        std::pow(gain, field.fieldExponent * sumAziElevSpans * distFromSource)};

    const float finalElevationGain{
        (gainFullElevSpan - gainNoSpan)
            * std::pow(sourceElevationSpan, finalElevSpanExponent * distFromSource)
        + gainNoSpan};

    const float finalAzimuthGain{
        (gainFullAzimuthSpan - gainNoSpan)
            * std::pow(sourceAzimuthSpan, finalAzimuthSpanExponent * distFromSource)
        + gainNoSpan};

    gains[i] = sumAziElevSpans * finalAzimuthGain + sumAziElevSpans * finalElevationGain
               + azimuthElevationGain;
  }

  auto const sum{
      std::reduce(gains, gains + field.speakerPositions.size(), 0.0f, std::plus())};

  if(sum > 0.0f)
  {

    auto const radius{std::sqrt(
        std::pow(source.position->getCartesian().x, 2.0f)
    ...
    std::transform(
        gains, gains + field.speakerPositions.size(), gains,
        [norm](float& gain) { return gain * norm; });
  }
}
}
...
MbapField mbapInit(SpeakersData const& speakers)
{
  ...
  std::transform(
      MbapSpeakers.cbegin(), MbapSpeakers.cend(), std::back_inserter(field.outputOrder),
