...
struct TripletData
{
  std::array<std::size_t, 3> tripletSpeakerNumber{};
  std::array<float, 9> tripletInverseMatrix{};
};

using triplet_list_t = std::vector<TripletData>;

static void computeGains(
    std::vector<SpeakerSet>& sets, SpeakersSpatGains& gains, int const numSpeakers,
    Position const& position, std::size_t const dim) noexcept
{
  if(sets.empty())
  {
    gains.fill(0.0f);
  ...
  if(sets[j].setGains[0] <= 0.0f && sets[j].setGains[1] <= 0.0f
     && sets[j].setGains[2] <= 0.0f)
  {
...
static bool testTripletContainsSpeaker(
    std::size_t const a, std::size_t const b, std::size_t const c,
...
static bool linesIntersect(
    std::size_t const i, std::size_t const j, std::size_t const k, std::size_t const l,
    std::array<Position, MAX_NUM_SPEAKERS> const& speakers) noexcept
{
  ...
  if(std::abs(distIv3) <= 0.01f || std::abs(distJv3) <= 0.01f
     || std::abs(distKv3) <= 0.01f || std::abs(distLv3) <= 0.01f
     || std::abs(distInv3) <= 0.01f || std::abs(distJnv3) <= 0.01f
     || std::abs(distKnv3) <= 0.01f || std::abs(distLnv3) <= 0.01f)
  {
    return false;
  }

  if((std::abs(distIj - (distIv3 + distJv3)) <= 0.01f
      && std::abs(distKl - (distKv3 + distLv3)) <= 0.01f)
     || (std::abs(distIj - (distInv3 + distJnv3)) <= 0.01f
         && std::abs(distKl - (distKnv3 + distLnv3)) <= 0.01f))
  {
    return true;
  }
  return false;
    ...
    SourceData const& source, SpeakersSpatGains& gains, VbapData& data) noexcept
{
  ...
  for(int i{}; i < NUM; ++i)
  {
    ...
    for(int k{}; k < kNum; ++k)
    {
      ...
      computeGains(
          data.speakerSets, tmpGains, data.numSpeakers, Position{spreadAngle},
          data.dimension);
      std::transform(
          gains.cbegin(), gains.cend(), tmpGains.cbegin(), gains.begin(),
...
static void
spreadGains2d(SourceData const& source, SpeakersSpatGains& gains, VbapData& data)
{
  int i;
  static constexpr auto NUM = 4;
  radians_t newAzimuth{};
  ...
  for(i = 0; i < NUM; i++)
  {
    ...
    radians_t const azimuthDeviation{
        degrees_t{narrow<float>(i + 1) * azimuthSpread * 45.0f}};
    for(int k{}; k < 2; ++k)
    {
      if(k == 0)
      {
...
static void sortSpeakers2d(
    std::array<Position, MAX_NUM_SPEAKERS>& speakers,
    ...
    std::size_t const numSpeakers)
{
  ...
  std::sort(
      sortedSpeakers.begin(), sortedSpeakers.begin() + numSpeakers,
...
static int
computeInverseMatrix2d(radians_t const azi1, radians_t const azi2, float invMat[4])
{
...
static void generateTuplets(
    std::array<Position, MAX_NUM_SPEAKERS>& speakers, triplet_list_t& triplets,
    std::size_t const numSpeakers)
{
  ...
  {
    if(speakers[sortedSpeakers[i + 1]].getPolar().azimuth
           - speakers[sortedSpeakers[i]].getPolar().azimuth
       <= MAX_SPREAD_ANGLE)
    {
      if(computeInverseMatrix2d(
             speakers[sortedSpeakers[i]].getPolar().azimuth,
             speakers[sortedSpeakers[i + 1]].getPolar().azimuth, inverseMatrix[i])
         != 0)
      {
        exist[i] = 1;
      }
  ...
  {
    if(computeInverseMatrix2d(
           speakers[sortedSpeakers[numSpeakers - 1]].getPolar().azimuth,
           speakers[sortedSpeakers[0]].getPolar().azimuth,
...
static float parallelepipedVolumeSideLength(
    Position const& i, Position const& j, Position const& k) noexcept
{
  auto const length = i.getCartesian().angleWith(j.getCartesian())
                      + i.getCartesian().angleWith(k.getCartesian())
...
static triplet_list_t generateTriplets(
    std::array<Position, MAX_NUM_SPEAKERS> const& speakers,
    std::size_t const numSpeakers)
{
  ...
  std::iota(
      std::begin(speakerIndexesSortedByElevation),
      std::end(speakerIndexesSortedByElevation), 0);

  auto const sortIndexesBySpeakerElevation
      = [speakers](size_t const& indexA, size_t const& indexB) -> bool {
    return speakers[indexA].getPolar().elevation < speakers[indexB].getPolar().elevation;
  };
  std::sort(
      std::begin(speakerIndexesSortedByElevation),
      std::end(speakerIndexesSortedByElevation), sortIndexesBySpeakerElevation);

  auto const connectionsPtr{std::make_unique<
      std::array<std::array<bool, MAX_NUM_SPEAKERS>, MAX_NUM_SPEAKERS>>()};
  ...
  for(size_t i{}; i < speakerIndexesSortedByElevation.size(); ++i)
  {
    ...
    for(auto j{i + 1}; j < speakerIndexesSortedByElevation.size(); ++j)
    {
      ...
      for(size_t k{}; k < speakerIndexesSortedByElevation.size(); ++k)
      {
        ...
        auto const parallelepipedVolume{
            parallelepipedVolumeSideLength(speaker1, speaker2, speaker3)};
        static constexpr auto MIN_VOL_P_SIDE_LENGTH = 0.01f;
        if(parallelepipedVolume > MIN_VOL_P_SIDE_LENGTH)
        {
          connections[narrow<std::size_t>(speaker1Index)]
                     [narrow<std::size_t>(speaker2Index)]
              = true;
          connections[narrow<std::size_t>(speaker2Index)]
                     [narrow<std::size_t>(speaker1Index)]
              = true;
          connections[narrow<std::size_t>(speaker1Index)]
                     [narrow<std::size_t>(speaker3Index)]
              = true;
          connections[narrow<std::size_t>(speaker3Index)]
                     [narrow<std::size_t>(speaker1Index)]
              = true;
          connections[narrow<std::size_t>(speaker2Index)]
                     [narrow<std::size_t>(speaker3Index)]
  ...
  for(std::size_t i{}; i < numSpeakers; ++i)
  {
    for(auto j{i + 1}; j < numSpeakers; ++j)
    {
      ...
      {
        auto const distance{
            speakers[i].getCartesian().angleWith(speakers[j].getCartesian())};
        std::size_t k{};
  ...
  auto const predicate
      = [&connections, speakers, numSpeakers](TripletData const& triplet) -> bool {
  ...
  auto const newTripletEnd{
      std::partition(std::begin(triplets), std::end(triplets), predicate)};
...
static void computeMatrices3d(
    triplet_list_t& triplets, std::array<Position, MAX_NUM_SPEAKERS>& speakers, int)
{
...
std::unique_ptr<VbapData> vbapInit(
    std::array<Position, MAX_NUM_SPEAKERS>& speakers, int const count,
    ...
    std::array<output_patch_t, MAX_NUM_SPEAKERS> const& outputPatches)
{
  ...
  for(auto const& triplet : triplets)
  {
    ...
    {
      newSet.speakerNos[j] = outputPatches
          [triplet.tripletSpeakerNumber[j] + narrow<std::size_t>(offset) - 1];
    }
    ...
    SourceData const& source, SpeakersSpatGains& gains, VbapData& data) noexcept
{
  ...
  computeGains(
      data.speakerSets, gains, data.numSpeakers, data.direction, data.dimension);
  if(data.dimension == 3)
  {
...
std::vector<Triplet> vbapExtractTriplets(VbapData const& data)
{
  ...
  for(auto const& set : data.speakerSets)
  {
    ...
    assert(
        isLegalOutputPatch(triplet.id1) && isLegalOutputPatch(triplet.id2)
